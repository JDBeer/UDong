...
static const CGFloat SVProgressHUDRingRadius = 16;
static const CGFloat SVProgressHUDRingNoTextRadius = 13;
...
+ (void)showHUDWithImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration
{
    [self showHUDWithImage:image status:string duration:duration maskType:SVProgressHUDMaskTypeClear];
}

+ (void)showHUDWithImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration maskType:(SVProgressHUDMaskType)hudMaskType
{
    [self setBackgroundColor:[UIColor blackColor]];
    [self setForegroundColor:[UIColor whiteColor]];
    [[self sharedView] showImage:image status:string duration:duration maskType:hudMaskType];
}

+ (void)showProgressWithStatus:(NSString *)string duration:(NSTimeInterval)duration
{
    [self showProgressWithStatus:string duration:duration maskType:SVProgressHUDMaskTypeClear];
}

+ (void)showProgressWithStatus:(NSString *)string duration:(NSTimeInterval)duration maskType:(SVProgressHUDMaskType)hudMaskType
{
    [self setBackgroundColor:[UIColor blackColor]];
    [self setForegroundColor:[UIColor whiteColor]];
    [[self sharedView] showProgress:SVProgressHUDUndefinedProgress status:string maskType:hudMaskType];
    if (duration != -1) {
        ...
        [[NSRunLoop mainRunLoop] addTimer:[self sharedView].fadeOutTimer forMode:NSRunLoopCommonModes];
    }
}
...
+ (void)showProgress
{
    [self setBackgroundColor:[UIColor blackColor]];
    [self setForegroundColor:[UIColor whiteColor]];
    [[self sharedView] showProgress:SVProgressHUDUndefinedProgress status:nil maskType:SVProgressHUDMaskTypeClear];
}

+ (void)showProgressWithClearBgColor
{
...
//            BOOL windowLevelNormal = window.windowLevel == UIWindowLevelNormal;
            // hud显示等级最高，显示时候无法触摸键盘
            BOOL windowLevelNormal = UIWindowLevelAlert;
